// Split terminal, first Run: node changeStreams.js, and then in other Run: changeStreams.js.  For triggers check Atlas in MongoDB
...
async function monitorListingsUsingEventEmitter(
  ...
  //* This is where you set the action you want to take place with the change stream, ex send an email, place an order, etc
...
//* Each change stream event document contains a "Resume Token" in the _id of the change event document. The application can pass the Resume Token when creating a new change stream to not lose any change stream events. But, the application should store the Resume Token becuase if the applicaction fails/restarts, the driver will lose it's cached copy.
...
async function monitorListingsUsingStreamAPI(
  ...
  // See https://mongodb.github.io/node-mongodb-native/3.6/api/ChangeStream.html#stream for the stream() docscle
